...
function TestQuestion(){
    const [strokeSize, setStrokeSize] = useState(parseFloat((window.innerWidth*0.4213/28).toFixed(2))-2);

    const pathElement= useRef(null);
    useEffect(() => {
        const handleResize = () => {
        const calculatedSize = parseFloat((window.innerWidth*0.4213/28).toFixed(2))-2;
        ...
        <div className={styles.container}>
            <div className={styles.questionZone}>
                ...
                <svg className={styles.borderBox} viewBox='0 0 300 100' preserveAspectRatio='none'>
                    <path ref={pathElement} d='M0,100 L300,100' vector-effect='non-scaling-stroke' style={{strokeDasharray: `${strokeSize}, 2`}}/>
                </svg>
                    <div className={styles.questionNum}>
                        <h4>1</h4>
                    </div>

                    <div className={styles.markReview}>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 15 15">
                            ...
                            clip-rule="evenodd" stroke="currentColor" strokeWidth="0.5px"/>
                        </svg>
